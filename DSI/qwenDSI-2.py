...
for index, row in d.iterrows():
    ...
    for i in range(len(sentences)):  # loop over sentences
        ...
        for j in sent_indices:  # loop over words and create list of all hidden vectors from layers 7 & 8; additionally store number of words (doubled, to account for layer 7 and 8 duplicates)
            words.append(sent_words[j])
            words.append(sent_words[j])
            features.append(layer7[0,j,:])  # layer 7 features
            features.append(layer8[0,j,:])  # layer 8 features
            print("Word: " + str(sent_words[j]) + " | Layer 7: " + str(layer7[0,j,:].numpy()) + " | Layer 8: " + str(layer8[0,j,:].numpy()))  # print word and its hidden activation vectors to console
    ...
    s[ID]["qwenDSI-2"] = mean_story_dcos
...
out_df.to_csv('QwenDSI-2_output.csv', index=False) # save updated dataframe
